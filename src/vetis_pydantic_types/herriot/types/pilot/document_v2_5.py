...
class AnimalLifecycleEventReason(BaseModel):
    """
    ...
    model_config = ConfigDict(defer_build=True)
    value: AnimalLifecycleEventReasonContentType = field(
        metadata={
            "required": True,
        }
    )
...
class AnimalRegistrationStatusChange(BaseModel):
    ...
    model_config = ConfigDict(defer_build=True)
    status: AnimalRegistrationStatus = field(
        metadata={
            "type": "Element",
            "namespace": "http://api.vetrf.ru/schema/cdm/mercury/vet-document/v2",
            "required": True,
...
class AnimalRegistration(GenericVersioningEntity):
    ...
    Attributes:
        ...
        specified_productivity: Сведения о продуктивности животного
            (группы животных)
        applicable_classification: Дополнительные характеристики учётной
            карточки
...
class ObservableAnimalGroup(BaseModel):
    ...
    animal_registration_ref: Optional[AnimalRegistration] = field(
        default=None,
        metadata={
